...
mod tests
{
    ...
    #[tokio::test]
    async fn tor_device_test(){
        get_singleton_multi();
        setup("TorDeviceTest".into());
        info!("Creating reservation!");
        let reservation = TorInterfaceConfig::new(TrafficStarInterfaceName::from_str("eth0").unwrap()).await;
        info!("Created reservation, creating device!");
